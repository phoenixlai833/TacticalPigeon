"use strict";
(self["webpackChunktactical_people"] = self["webpackChunktactical_people"] || []).push([[6],{
...
/* harmony export */   "soundMuteEvent": () => (/* binding */ soundMuteEvent),
...
function playSound(_x) {
  return _playSound.apply(this, arguments);
}
function _playSound() {
  _playSound = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(sound) {
    var audio;
...
var soundMuteEvent = new CustomEvent('soundMuteToggle');
document.addEventListener('soundMuteToggle', function (event) {
  var detail = event.detail;
  if (!detail) {
    return;
  }
  var soundsToMute = detail.soundsToMute;
  soundsToMute.map(function (sound) {
    if (sound.mute !== undefined) sound.mute = !sound.mute;else sound.mute = true;
    if (sound.audioElement) {
      sound.audioElement.muted = !sound.audioElement.muted;
    }
    if (sound.audio) {
      sound.audio.mute = !sound.audio.mute;
      if (sound.mute) sound.audio.pause();
    }
  });
});
